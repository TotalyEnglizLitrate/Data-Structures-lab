...
void List::reverse(void) {
    if (this->len <= 1)
        return;
    ListNode *start, *post_start, *end, *pre_end;
    start = this->head;
    end = this->tail;
    ...
    while (i < this->len / 2) {
        ...
        pre_end = end->get_prev();

        pre_end->set_next(start);
        post_start->set_prev(end);

        // If end's next is nullptr, start's prev is going to be nullptr because we're moving outside in
        if (end->get_next() == nullptr) {
            start->set_next(nullptr);
            end->set_prev(nullptr);
        }
        else {
            start->set_next(end->get_next());
            start->get_next()->set_prev(start);
            end->set_prev(start->get_prev());
            end->get_prev()->set_next(end);
        }

        start->set_prev(pre_end);
